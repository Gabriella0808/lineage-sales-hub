...
// Static seed data mirroring KPI_2026.01.15_Live.xlsx -啋 Summary tab
...
// Maps display rep names (REP_BOOK) -啋 keys in REP_MONTHLY (spreadsheet tabs).
...
// Maps Live KPI display rep names -啋 matching name(s) in the sales_reps table
...
// Maps REP_BOOK display names -啋 list of territory names they cover.
...
export function LiveKpiReport({ managerName, lockedRepName }: { managerName?: string; lockedRepName?: string | null } = {}) {
  ...
  useEffect(() => {
    ...
    (async () => {
      // Resolve scoped rep names -啋 dealer_ids (only when a scope is active).
  ...
  const scaledMonthly = useMemo(() => {
    ...
    // Priority: explicit rep selection -啋 territory filter -啋 manager scope -啋 all.
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-baseline justify-between mb-3">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              ...
              {hasRepSelection
                ? <> 路 <span className="font-medium text-foreground">{repFilter.length === 1 ? repFilter[0] : `${repFilter.length} reps`}</span></>
                : <> 路 all reps</>}
            </p>
          </div>
      ...
      <div className="glass-card p-5">
        ...
        <p className="text-xs text-muted-foreground mb-4">
          ...
          {hasRepSelection
            ? <span className="ml-1">路 live data from <span className="font-medium text-foreground">{repFilter.length === 1 ? `${repFilter[0]} tab` : `${repFilter.length} reps`}</span></span>
            : <> 路 all reps</>}
        </p>
