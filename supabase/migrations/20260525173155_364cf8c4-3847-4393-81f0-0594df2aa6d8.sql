...
WHERE l.invoice_date IS NOT NULL
  ...
  -- Exclude ONLY tariffs, freight and clearance/closeout lines. Everything
  -- else (surcharges, taxes, fees, special orders, etc.) is counted.
  AND NOT (
       coalesce(lower(l.sku), '')          ~ 'tariff|freight|clearance|closeout'
    OR coalesce(lower(l.product_name), '') ~ 'tariff|freight|clearance|closeout'
  )
GROUP BY 1, 2, 3;
