-- Exclude freight/tariff/surcharge lines from open orders in both rollup functions.
-- Previously used dbo_Orders.SubTotal (includes all charges).
-- Now joins dbo_OrderDetail and applies same filter as dbo_InvoiceDetail:
--   Freight = false, LineCancelled = false, MiscChargeType is blank.

-- 1. Booking rollup: aggregate from OrderDetail instead of Orders.SubTotal
...
AS $$
  SELECT
    ...
    COALESCE(SUM(od."Amount"::numeric), 0)           AS bookings,
    COUNT(DISTINCT o."GUIDOrder")::int               AS booking_count,
    COALESCE(SUM(CASE WHEN o."BranchID" = 'MIXED'   THEN od."Amount"::numeric ELSE 0 END), 0) AS bookings_container,
    COALESCE(SUM(CASE WHEN o."BranchID" = 'WHSALES' THEN od."Amount"::numeric ELSE 0 END), 0) AS bookings_warehouse
...
-- 2. Invoice rollup: same fix for the open-orders portion
...
AS $$
  ...
  order_agg AS (
    SELECT
      EXTRACT(YEAR  FROM o."OrderDate")::int AS year,
      EXTRACT(MONTH FROM o."OrderDate")::int AS month,
      COALESCE(SUM(od."Amount"::numeric), 0) AS total,
      COALESCE(SUM(CASE WHEN o."BranchID" = 'MIXED'   THEN od."Amount"::numeric ELSE 0 END), 0) AS total_container,
      COALESCE(SUM(CASE WHEN o."BranchID" = 'WHSALES' THEN od."Amount"::numeric ELSE 0 END), 0) AS total_warehouse,
