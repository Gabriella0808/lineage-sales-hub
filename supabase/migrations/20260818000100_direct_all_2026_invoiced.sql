-- Route ALL 2026 invoiced data through acctivate_invoice_lines_2026_direct.
--
-- Skyvia has hit its record limit so portal_acctivate_invoice_lines (pail) and
-- portal_acctivate_invoices (pai) are stale for August+. The direct pull script
-- (pull-2026-invoiced-lines-direct.ps1) now owns ALL 2026 invoice lines,
-- including August, and upserts them into acctivate_invoice_lines_2026_direct.
--
-- Changes:
--  1. Add columns to acctivate_invoice_lines_2026_direct that the new script populates.
--  2. Add refresh_mv_portal_invoiced() so the PS1 script can refresh after upload.
--  3. Rebuild v_portal_invoice_line_facts – all 2026 from the direct table (no pail/pai
--     for Aug+). Rep name resolved from acctivate_sales_reps, then pai fallback.
--  4. Rebuild mv_portal_monthly_invoiced_actuals – Aug+ from the direct table,
--     Jan–Jul still from the pre-verified acctivate_kpi_monthly_invoiced_2026 total.
--  5. Rebuild get_portal_invoiced_lines() and v_portal_dealer_rep_reporting_lines
--     (signatures unchanged).
--
-- No source data modified. Bookings, projections, and 2025 actuals are untouched.

-- ─────────────────────────────────────────────────────────────────────────────
-- 1. Add new columns to the direct pull table
-- ─────────────────────────────────────────────────────────────────────────────
...
ALTER TABLE public.acctivate_invoice_lines_2026_direct
  ...
  ADD COLUMN IF NOT EXISTS formula_net_amount numeric,
  ADD COLUMN IF NOT EXISTS product_class      text;

-- ─────────────────────────────────────────────────────────────────────────────
-- 2. Refresh function (called by PS1 script after each upload)
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 3. Tear down dependency chain (deepest first)
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 4. v_portal_invoice_line_facts
--    All 2026 from acctivate_invoice_lines_2026_direct (no pail/pai for Aug+).
--    Rep name: acctivate_sales_reps first, pai fallback, then rep_id, then
--    'Unassigned'.
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 5. mv_portal_monthly_invoiced_actuals
--    Jan–Jul: pre-verified totals from acctivate_kpi_monthly_invoiced_2026
--             + branch split from the direct table (same as before)
--    Aug+:    aggregated directly from acctivate_invoice_lines_2026_direct
--             (pail/pai no longer used for Aug)
-- ─────────────────────────────────────────────────────────────────────────────
...
WITH jan_jul_branch AS (
  SELECT
    ...
    SUM(CASE
      ...
      THEN COALESCE(d.invoice_detail_amount, 0)::numeric ELSE 0
...
aug_on AS (
  SELECT
    d.year,
    d.month_number,
    -- Total: all product-category lines (matches KPI methodology for Jan–Jul)
    SUM(CASE
...
-- Jan–Jul: use pre-verified monthly total, branch split from direct table
SELECT
  k.year,
  k.month_number,
  k.invoiced_actual,
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 6. get_portal_invoiced_lines()  — unchanged signature, reads updated view
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 7. v_portal_dealer_rep_reporting_lines — unchanged
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 8. v_companywide_reporting_actuals — recreate (unchanged logic)
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 9. RPCs that depend on v_companywide_reporting_actuals — recreate unchanged
-- ─────────────────────────────────────────────────────────────────────────────
