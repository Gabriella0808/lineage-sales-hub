-- Fix net_booking_amount formula in v_portal_bookings_line_facts.
--
-- Problem: original_price is uploaded as "0" or "0.0000" (non-empty string).
-- Old condition: NULLIF(original_price, '') IS NOT NULL
--   → true for "0" → qty * 0 * discount = 0
-- Fix: cast to numeric and reject zero:
--   COALESCE(NULLIF(original_price, '')::numeric, 0) <> 0
--
-- Cannot use CREATE OR REPLACE VIEW — the live view has extra columns from the
-- original Supabase schema. Must DROP CASCADE and recreate the full chain.
--
-- Drop order (deepest dependent first):
...
-- Then recreate each in reverse order.

BEGIN;

-- ─────────────────────────────────────────────────────────────────────────────
-- 1. Drop dependency chain
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 2. v_portal_bookings_line_facts  (fixed formula)
--
--    Formula:
--      if original_price is a non-zero number →
--        qty_ordered * original_price * (1 - line_discount_pct / 100)
--      else →
--        amount - tariff_amount - freight_amount
--
--    "Non-zero number" check:
--      COALESCE(NULLIF(original_price, '')::numeric, 0) <> 0
--      handles: NULL → 0, '' → 0, '0' → 0, '0.0000' → 0, '125.5' → 125.5
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 3. v_portal_dealer_rep_reporting_lines  (unchanged from 20260817000200)
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 4. v_companywide_reporting_actuals  (unchanged from 20260818000300)
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 5. mv_portal_monthly_net_bookings_actuals  (unchanged from 20260814006000)
--    Built WITH DATA — August totals included immediately.
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 6. refresh_mv_portal_bookings  (recreate after mat view was dropped)
-- ─────────────────────────────────────────────────────────────────────────────
...
-- Mat view already built WITH DATA above; notify PostgREST to reload cache.
