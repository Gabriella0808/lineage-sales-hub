-- Fix invoice sales_category filter:
--   Old: IN ('SW', 'FINNLOU', 'LUX', 'ALLOW')
--   New: COALESCE(sales_category, 'NULL') IN ('NULL', 'SW', 'ALLOW', 'FL', 'LUX')
--
-- Changes:
--   • FINNLOU removed — Acctivate now uses FL for Finn & Lou
--   • FL added
--   • NULL category rows now included (allowances/credits with no category set)
--   • MISC excluded (was never in the old list; confirmed: ALLOW → display 'MISC' was
--     misleading — ALLOW now displays as 'ALLOW')
--   • display_category for ALLOW changed from 'MISC' to 'ALLOW'
--
-- Affected objects (dropped and recreated in dependency order):
--   mv_portal_monthly_invoiced_actuals  (mat view — direct dependency on staging table)
...
--
-- Does NOT change bookings filters, 2025 actuals, or Jan–Jul pre-verified totals.

-- ─────────────────────────────────────────────────────────────────────────────
-- Tear down (outermost first)
-- ─────────────────────────────────────────────────────────────────────────────
...
WHERE d.invoice_date IS NOT NULL
  ...
  AND COALESCE(d.product_sales_category, 'NULL') IN ('NULL', 'SW', 'ALLOW', 'FL', 'LUX');
...
-- Aug+:    aggregated from direct table
-- ─────────────────────────────────────────────────────────────────────────────
...
jan_jul_count AS (
  ...
  WHERE invoice_date BETWEEN '2026-01-01' AND '2026-07-31'
    AND COALESCE(product_sales_category, 'NULL') IN ('NULL', 'SW', 'ALLOW', 'FL', 'LUX')
...
aug_on AS (
  SELECT
    ...
    d.month_number,
    SUM(CASE
      WHEN COALESCE(d.product_sales_category, 'NULL') IN ('NULL', 'SW', 'ALLOW', 'FL', 'LUX')
      THEN COALESCE(d.formula_net_amount, 0)::numeric ELSE 0
...
-- ─────────────────────────────────────────────────────────────────────────────
-- v_portal_dealer_rep_reporting_lines  (latest: uses f.product_class for bookings)
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────────
-- Refresh materialized view so data is immediately live
-- ─────────────────────────────────────────────────────────────────────────────
