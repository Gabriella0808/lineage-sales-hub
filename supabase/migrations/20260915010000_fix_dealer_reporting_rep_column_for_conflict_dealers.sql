-- ══════════════════════════════════════════════════════════════════════════════
-- Fix: rep-scoped Dealer Reporting's REP column showed the dealer's ROSTER-
-- assigned rep even for rows that only appear because of a DIFFERENT rep's
-- own transaction-coded sales (the "conflict dealer" case from the earlier
-- transaction-rep-scoping migration, e.g. Rock's Carolina Furniture: roster
-- says Dave Ervin, but Brad Robertson is the one who actually sold there).
--
-- Concretely: when Brad Robertson views his own Dealer Reporting, Rock's
-- Carolina Furniture shows up (correctly - Brad's own coded sales are
-- there), but the REP column showed "Dave Ervin" - confusing, and reported
-- as looking like another rep's data leaking into Brad's own view. It's
-- not a leak (the dollar amount is genuinely Brad's), but the column was
-- showing the wrong name for it: it should say who actually sold there.
--
-- Fix: for rep-scoped views, when a row's $ amount came from a rep's own
-- transaction-coded sales (rep_filtered_agg) rather than a plain roster
-- match, the REP column now shows the transacting rep's name(s) instead of
-- the roster owner. For a dealer's normal $0/roster-only rows, and for the
-- fully unfiltered admin view, nothing changes - same roster rep name as
-- before. If a manager filters by multiple reps and a dealer has sales
-- from more than one of them, all contributing names are shown
-- (comma-separated) rather than picking just one arbitrarily.
--
-- No changes to dealer roster ownership, territory, amounts, Open SO, or
-- any other calculation - purely which name is displayed in this one
  ...
  IF p_group_by = 'dealer' THEN
    ...
    rep_filtered_src AS (
      SELECT
        ...
        a.fulfillment_type,
        a.transaction_date,
        -- NEW: who actually made this sale, same derivation Rep Reporting uses
        COALESCE(
    ...
    rep_filtered_agg AS (
      SELECT
        ...
        -- NEW: the transacting rep name(s) for this dealer, for display
    ...
    SELECT
      ...
      -- CHANGED: for rep-scoped rows backed by this rep's own transactions,
      -- show who actually sold here instead of the roster owner. Falls
      -- back to the roster name for $0 roster-only rows and for the
      -- unfiltered admin view (rfa.txn_rep_names is NULL in both cases).
      COALESCE(
